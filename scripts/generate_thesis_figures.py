...
def fig_a2_reliability_panel():
    ...
    panel_models = [('image', 'Image-only (DenseNet121)'), ('multimodal', 'Multimodal-concat'), ('attention', 'Multimodal-attention'), ('lr', 'Clinical LR')]
...
def fig_a5_training_curves():
    ...
    histories = {}
    for key in ['image', 'multimodal']:
        with open(HISTORY_FILES[key]) as f:
            histories[key] = json.load(f)
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
    titles = {'image': 'Image-only Fine-tuning (DenseNet121)', 'multimodal': 'Multimodal-concat Fine-tuning'}
    for ax, (key, hist) in zip(axes, histories.items()):
        ...
        ax2 = ax.twinx()
        ax2.plot(epochs, va, 's-', color='#CC6600', lw=2.0, ms=5, label='Val AUPRC')
        ...
        ax2.annotate(f'Best epoch {best_ep}\nAUPRC={best_auprc:.4f}', xy=(best_ep, best_auprc), xytext=(best_ep + 0.6, best_auprc - 0.04), fontsize=8.5, color='#333333', arrowprops=dict(arrowstyle='->', color='#333333', lw=0.8))
        ax.set_title(titles[key], fontsize=11, fontweight='bold')
        ax.grid(True, alpha=0.25)
        lines1 = [matplotlib.lines.Line2D([0], [0], color=color, lw=2, linestyle='-'), matplotlib.lines.Line2D([0], [0], color=color, lw=2, linestyle='--'), matplotlib.lines.Line2D([0], [0], color='#CC6600', lw=2, linestyle='-', marker='s')]
        ax.legend(lines1, ['Train loss', 'Val loss', 'Val AUPRC'], loc='upper right', fontsize=9)
    fig.suptitle('Training Curves — Fine-tuning Stage (val_AUPRC selection criterion)', fontsize=12)
    fig.tight_layout()
...
def fig_a7_vital_distributions():
    ...
    fig.legend(handles=patches, loc='lower center', ncol=2, fontsize=11, bbox_to_anchor=(0.5, -0.01))
...
def fig_a8_label_distribution():
    ...
    for ax, sp in zip(axes, splits):
        ...
        ax.grid(True, axis='y', alpha=0.25)
        ax.spines[['top', 'right']].set_visible(False)
    fig.suptitle('Label Distribution by Policy and Split — CheXpert Uncertainty Handling', fontsize=12)
    fig.tight_layout()
