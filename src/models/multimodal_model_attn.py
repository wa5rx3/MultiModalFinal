...
class MultimodalPneumoniaModelAttn(nn.Module):
    """Multimodal fusion using a lightweight cross-modal Transformer.

    Architecture:
        DenseNetBackbone → 1024-dim → Linear → D_model
        TabularMLP       → hidden_dim → Linear → D_model

        [CLS, image_token + type_embed[0], tabular_token + type_embed[1]]
        → TransformerEncoder (n_layers, nhead, dim_feedforward)
        → CLS output → LayerNorm → Linear → 1 logit
    """
    ...
    def __init__(
        ...
        # Type embeddings: image token = 0, tabular token = 1
    ...
    def forward(self, image: torch.Tensor, tabular: torch.Tensor) -> torch.Tensor:
        image_feat = self.image_backbone(image)   # [B, 1024]
        tab_feat = self.tabular_branch(tabular)   # [B, tabular_hidden_dim]

        image_tok = self.image_proj(image_feat)   # [B, d_model]
        tab_tok = self.tabular_proj(tab_feat)     # [B, d_model]
        ...
        # Sequence: [CLS, image, tabular] → [B, 3, d_model]
        ...
        return self.classifier(self.norm(cls_out))  # [B, 1]

    # --- backbone utility methods (same interface as MultimodalPneumoniaModel) ---
